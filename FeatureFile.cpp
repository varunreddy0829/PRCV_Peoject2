...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    std::cerr << "Usage: " << argv[0]
              << " <Directory> <FeatureType> <OutputCSV>" << std::endl;
    std::cerr << "Available Types: BaselineMatching, Histogram (TBD)"
              << std::endl;
    return -1;
  }
  ...
  if (!fs::exists(dirPath) || !fs::is_directory(dirPath)) {
    std::cerr << "Error: Directory " << dirPath << " does not exist."
              << std::endl;
    return -1;
  }
  ...
  for (const auto &entry : fs::directory_iterator(dirPath)) {
    ...
    } else if (featureType == "CustomTask7") { // New Task 7
      status = extractTask7Features(img, features);
    } else if (featureType == "GaborTexture") { // Extension 1
      status = extractGaborFeatures(img, features);
    } else {
  ...
  std::cout << "Done! Processed " << count << " images using " << featureType
            << std::endl;
  return 0;
}